...
Пакет регистрирует команду `cache:prune`, если приложение использует
`phpsoftbox/cli-app` и package command discovery:
...
Команда удаляет expired-записи, битые cache-файлы и старые временные файлы.
Permanent-записи без TTL удаляются только при явном `--max-age-days`.
